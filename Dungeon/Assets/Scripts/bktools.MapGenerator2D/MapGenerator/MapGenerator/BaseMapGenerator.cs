...
public abstract class BaseMapGenerator : IMapGenerator
{
    ...
    protected void BuildSubWalls() => CheckAndSetNeighborWalls(CellType.Empty, CellType.SubGate, CellType.Wall);
    ...
    protected void BuildGate()
    {
        Func<Vector2Int, bool> isGateCandidate = pos =>
            ...
            GetNeighborCells(pos)
                .Any(cell => cell == CellType.Floor || cell == CellType.FloorCenter || cell == CellType.Wall) &&
            GetNeighborCells(pos)
                .Any(cell => cell == CellType.ExpandedPath || cell == CellType.Path) &&
            GetNeighborCells(pos)
                .Any(cell => cell == CellType.MainGate) && 
            GetNeighborCells(pos)
                .Any(cell => cell == CellType.Wall || cell == CellType.PathWall) && 
            GetNeighborCells(pos)
                .Any(cell => cell != CellType.Empty);

        ProcessGrid(isGateCandidate, CellType.SubGate);
    }
        ...
        foreach (var pos in path)
        {
            ...
            else if (_grid[pos.x, pos.y] == CellType.Wall)
            {
                ...
                for (int i = 0; i < 4; i++)
                {
                    ...
                    {
                        if (_grid[nx, ny] == CellType.MainGate)
                        {
                            isMainGateNeighbor = true;
                            break;
                        }
                    }
                }

                if (!isMainGateNeighbor)
                {
    ...
    {
        if (IsValid(pos) && _grid[pos.x, pos.y] == CellType.MainGate)
        {
            // 게이트 방향을 얻습니다.
